    ...
    function checkId()
{
        ...
        type: "POST", 
                ...
                data:{id:"<?php echo $model->id;?>",table:"office"},
                success: function(msg){	
            
                        
                        if(msg=='0'){ 
                                window.location='<?php echo Yii::app()->baseUrl;?>/adminoffice';
                        }
                }
        });
}
    jQuery(function($) {
        ...
        $("#office_form").attr('action', '<?php echo Yii::app()->baseUrl; ?>/adminoffice/editconfirm/');

        //check back browser 	    
        branch_name = getCookie("office_edit_division_name");
...
//        }
        zipcode = getCookie("office_edit_zipcode");
        if (zipcode != null && zipcode != "null") {
            $("#Office_zipcode").val(zipcode);
        }


        address = getCookie("office_edit_address");
        if (address != null && address != "null") {
            $("#Office_address").val(address);
        }
        googlemap = getCookie("office_edit_googlemap");
        if (googlemap != null && googlemap != "null") {
            googlemap=googlemap.replace(/<br ?\/?>|_/g, '\n');
            $("#Office_googlemap").val(googlemap);
        }
        photo_checkbox_for_deleting = getCookie("office_edit_photo_checkbox_for_deleting");
        ...
        if (photo_checkbox_for_deleting != null && photo_checkbox_for_deleting != "null") {
            if (photo_checkbox_for_deleting == '1') {
                ...
                $fileInput=$("#Office_photo_checkbox_for_deleting").parent().prev();
              name=$fileInput.attr('name');
              id=$fileInput.attr('id');
              classAttr=$fileInput.attr('class'); 
              if(name==undefined){
              ...
              $fileInput.replaceWith("<input type='file' name='"+name+"' id='"+id+"' class='"+classAttr+"'/>");
              //
              $node1=$("#Office_photo_checkbox_for_deleting").parent().parent().prev().prev();
              $node1.remove();
              $('<img alt="" src="<?php echo $this->assetsBase; ?>/css/common/img/img_building.jpg">').insertBefore($("#Office_photo_checkbox_for_deleting").parent().parent().prev());                      
            }
            else {
        ...
        $('button[type="submit"]').click(function() {
            ...
            $.ajax({
                type: "POST",
                async: true,
                url: "<?php echo Yii::app()->baseUrl; ?>/adminoffice/edit/",
                ...
                success: function(msg)
                {
                    ...
                    {
                        setCookie("office_edit_division_name", $("#Office_division_name").val());

                        setCookie("office_edit_zipcode", $("#Office_zipcode").val());

                        setCookie("office_edit_address", $("#Office_address").val());
                        val=$("#Office_googlemap").val();
                        val=val.replace(/\n/g, "<br/>");
                        setCookie("office_edit_googlemap", val);
                        ...
                        if ($("#Office_photo_checkbox_for_deleting").is(':checked')) {
                            ...
                            setCookie("office_edit_photo_checkbox_for_deleting", '1');
                        }
                        else {

                            setCookie("office_edit_photo_checkbox_for_deleting", '0');
                        }
...
<div class="wrap admin secondary office">
    ...
    <div class="container index">
        <div class="contents detail edit">
                    ...
                    <div class="cnt-box">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                ...
                                $attachement4->editOffice($model, $form, 'adminoffice', $attachment4_error, $this->assetsBase);
                                $this->endWidget();
                                ?>
                            </div>
                        </div>
