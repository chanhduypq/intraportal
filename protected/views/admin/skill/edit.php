            ...
            <div class="mainBox detail">
				...
				<h2>資格取得・スキルアップ！ - 修正</h2>
                 <?php 
					...
					?>
                <span><a class="btn btn-important" href="<?php echo Yii::app()->request->baseUrl; ?>/adminskill/<?php echo $page; ?>"><i class="icon-chevron-left icon-white"></i> 一覧に戻る</a></span>
                </div>
                <div class="box">
                <?php
                ...
                <div class="cnt-box">
                  ...
                  <div class="field attachements">
                        ...
                        $attachements->edit11($model, $form,'adminskill',$attachment1_error,$attachment2_error,$attachment3_error,$this->assetsBase);                        
                        $this->endWidget();
                        ?>
...
<script type="text/javascript">       
        ...
        jQuery(function($){ 
			...
			$("body").attr('id','admin');  
           $("#skill_form").attr('action', '<?php echo Yii::app()->baseUrl; ?>/adminskill/editconfirm/');                    
 			
		   category_id=getCookie("skill_edit_category_id");
		
           if(category_id!=null&&category_id!="null"){
               $("#Skill_category_id").val(category_id);
			  
           }  
           title=getCookie("skill_edit_title");
		   ...
		   url=getCookie("skill_edit_url");
           if(url!=null&&url!="null"){
               $("#Skill_url").val(url);
           }
           comment=getCookie("skill_edit_comment");           
           if(comment!=null&&comment!="null"){
              comment1=comment.replace(/<br ?\/?>|_/g, '\n');    
               $("#Skill_comment").val(comment1);
           }
           attachment1_checkbox_for_deleting=getCookie("skill_edit_attachment1_checkbox_for_deleting");
           if(attachment1_checkbox_for_deleting!=null&&attachment1_checkbox_for_deleting!="null"){              
               if(attachment1_checkbox_for_deleting=='1'){
                   ...
                   $fileInput=$("#Skill_attachment1_checkbox_for_deleting").parent().parent().prev().find('input[type="file"]').eq(0);
                  name=$fileInput.attr('name');
                  id=$fileInput.attr('id');
                  classAttr=$fileInput.attr('class');
                  ...
                  $fileInput.replaceWith("<input type='file' name='"+name+"' id='"+id+"' class='"+classAttr+"'/>");
                  //
                  $node1=$("#Skill_attachment1_checkbox_for_deleting").parent().parent().prev().prev();
                  $node1.remove();
                  $('<img alt="" src="<?php echo $this->assetsBase; ?>/css/common/img/img_photo01.jpg">').insertBefore($("#Skill_attachment1_checkbox_for_deleting").parent().parent().prev());
               }
               else{
           ...
           attachment2_checkbox_for_deleting=getCookie("skill_edit_attachment2_checkbox_for_deleting");
           if(attachment2_checkbox_for_deleting!=null&&attachment2_checkbox_for_deleting!="null"){
               if(attachment2_checkbox_for_deleting=='1'){
                   ...
                   $fileInput=$("#Skill_attachment2_checkbox_for_deleting").parent().parent().prev().find('input[type="file"]').eq(0);
                  name=$fileInput.attr('name');
                  id=$fileInput.attr('id');
                  classAttr=$fileInput.attr('class');
                  ...
                  $fileInput.replaceWith("<input type='file' name='"+name+"' id='"+id+"' class='"+classAttr+"'/>");
                  //
                  $node1=$("#Skill_attachment2_checkbox_for_deleting").parent().parent().prev().prev();
                  $node1.remove();
                  $('<img alt="" src="<?php echo $this->assetsBase; ?>/css/common/img/img_photo01.jpg">').insertBefore($("#Skill_attachment2_checkbox_for_deleting").parent().parent().prev());
               }
               else{
                   $("#Skill_attachment2_checkbox_for_deleting").attr('checked',false);
               }
           }
           attachment3_checkbox_for_deleting=getCookie("skill_edit_attachment3_checkbox_for_deleting");
           if(attachment3_checkbox_for_deleting!=null&&attachment3_checkbox_for_deleting!="null"){
               if(attachment3_checkbox_for_deleting=='1'){
                   ...
                   $fileInput=$("#Skill_attachment3_checkbox_for_deleting").parent().parent().prev().find('input[type="file"]').eq(0);
                  name=$fileInput.attr('name');
                  id=$fileInput.attr('id');
                  classAttr=$fileInput.attr('class');
                  ...
                  $fileInput.replaceWith("<input type='file' name='"+name+"' id='"+id+"' class='"+classAttr+"'/>");
                  //
                  $node1=$("#Skill_attachment3_checkbox_for_deleting").parent().parent().prev().prev();
                  $node1.remove();
                  $('<img alt="" src="<?php echo $this->assetsBase; ?>/css/common/img/img_photo01.jpg">').insertBefore($("#Skill_attachment3_checkbox_for_deleting").parent().parent().prev());
               }
               else{
           ...
           setCookie("skill_edit_comment",comment); 
			...
			$.ajax({ 
				...
				type: "POST", 
				async:true,
				url: "<?php echo Yii::app()->baseUrl;?>/adminskill/edit/?id=<?php echo $model->id;?>",     
				...
				success: function(msg){	                        
                      ...
                      jQuery('#Skill_url').prev().remove();   
					  	...
					  	if(msg!='[]'|checkFile()==false){
                                                ...
                                                val=val.replace(/\n/g, "<br/>");
                                                setCookie("skill_edit_comment",val);
                                                if($("#Skill_attachment1_checkbox_for_deleting").attr('checked')==true){
                                                    setCookie("skill_edit_attachment1_checkbox_for_deleting",'1');
                                                }
                                                else{
                                                    setCookie("skill_edit_attachment1_checkbox_for_deleting",'0');
                                                }
                                                if($("#Skill_attachment2_checkbox_for_deleting").attr('checked')==true){
                                                    setCookie("skill_edit_attachment2_checkbox_for_deleting",'1');
                                                }
                                                else{
                                                    setCookie("skill_edit_attachment2_checkbox_for_deleting",'0');
                                                }
                                                if($("#Skill_attachment3_checkbox_for_deleting").attr('checked')==true){
                                                    setCookie("skill_edit_attachment3_checkbox_for_deleting",'1');
                                                }
                                                else{
                                                    setCookie("skill_edit_attachment3_checkbox_for_deleting",'0');
                                                }
...
function checkId()
{
	...
	type: "POST", 
			async:true,
			url: "<?php echo Yii::app()->baseUrl;?>/adminskill/checkId/",    
			data:{id:"<?php echo $model->id;?>",table:"skill"},
			success: function(msg){	
					if(msg=='0'){ 
							window.location='<?php echo Yii::app()->baseUrl;?>/adminskill';
					}
			}
	});
}
