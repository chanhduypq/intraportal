    ...
    <div class="container">
        <div class="contents regist">
            ...
            <div class="mainBox detail">
            	<div class="pageTtl"><h2>総務からのお知らせ：人事 - 登録</h2>
                <?php 
...
<script type="text/javascript">  
		...
		if (navigator.userAgent.indexOf('Chrome') != -1){        
			...
			if(from=="confirmedit"){
				...
				window.location="<?php echo Yii::app()->baseUrl.'/adminsoumu_jinji/regist'?>"; 
			}
    	}   
		jQuery(function($) {        
			$("body").attr('id','admin');  
		   ...
		   deleteCookies("from_jini");         
			$.ajax({    
				type: "POST", 
				async:true,
				url: "<?php echo Yii::app()->baseUrl;?>/adminsoumu_jinji/regist/",    
				data: jQuery('#soumu_jinji_form').serialize(),
				success: function(msg){	            
					    jQuery('#soumu_jinji_form input,#soumu_jinji_form select[id="Soumu_jinji_category_id"]').prev().remove();
					  	if(msg!='[]'){
                                                    data=$.parseJSON(msg);
		   ...
		   $(window).load(function()
             {
				from=getCookie("from_jini");
			 	 if(from==null||from!="confirmedit"){ 
					$("#soumu_jinji_form").attr('action','<?php echo Yii::app()->baseUrl;?>/adminsoumu_jinji/registconfirm/');
						jQuery('#soumu_jinji_form').attr('onsubmit','return true;');
