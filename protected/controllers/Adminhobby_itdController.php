...
class Adminhobby_itdController extends Controller {    
    ...
    public function actionIndex() {
		...
		$this->render('/admin/hobby_itd/index',array(
	...
	public function actionEditconfirm() { 
		 ...
		 if ($model== null) 
			...
			if (Yii::app()->request->isPostRequest){  
				...
				if (!isset($_POST['edit']) || $_POST['edit'] != '1') {
					Upload_file_common_new::processAttachments($model, 'hobby_itd', 2);
				} 
				if ($model->id == null || $model->id == '') {
				 ...
				 if ($model->validate()) {
					...
					if ($model->attachment1 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment1)) {
						if ($model->attachment1 != Upload_file_common::getAttachmentById($model->id, 1,'hobby_itd')) {
					...
					if ($model->attachment2 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment2)) {
						if ($model->attachment2 != Upload_file_common::getAttachmentById($model->id, 2,'hobby_itd')) {
					...
					if ($model->attachment3 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment3)) {
						if ($model->attachment3 != Upload_file_common::getAttachmentById($model->id, 3,'hobby_itd')) {
					...
					if ($model->eye_catch != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->eye_catch)) {
                                                if ($model->eye_catch != $model->getEyeCatchById($model->id)) {
        ...
        $this->render('/admin/hobby_itd/editconfirm', array('model' => $model)); 
    } 
	 /**
     * delete file fordel
     */
    ...
    public function actionDetail()
	{
			...
			{ 
				$this->render('/admin/hobby_itd/detail', array(
					'model' => $model,
						)
				);
			}
     ...
     */
     public function actionDelete() {
        ...
        $affected_hobby_itd=$model->deleteByPk($id);
	...
	public function filters() {
        $backCookie = Yii::app()->request->cookies->contains('hobby_itd_edit_from') ? Yii::app()->request->cookies['hobby_itd_edit_from']->value : '';
