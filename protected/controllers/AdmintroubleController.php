...
class AdmintroubleController extends Controller {
    ...
    public function filters() {
        $backCookie = Yii::app()->request->cookies->contains('trouble_edit_from') ? Yii::app()->request->cookies['trouble_edit_from']->value : '';
    ...
    public function actionIndex() {
        ...
        $this->render('/admin/trouble/index', array('troubles' => $model, 'pages' => $pages));
    }
     ...
     * Author:
     * User change:
     * Description: edit trouble object
     * */
    ...
    public function actionEdit() {
        ...
        if ($model == null || !isset($_GET['id'])) {
            $this->redirect(array('admintrouble/index'));
        }
        ...
        $this->render('/admin/trouble/edit', $parmas);
    }
    ...
    public function actionEditconfirm() {
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            if (!isset($_POST['edit']) || $_POST['edit'] != '1') {
                ...
                Upload_file_common_new::processAttachments($model,'trouble',2);
            }
            ...
            if ($model->validate()) {
                if (isset($_POST['edit']) && $_POST['edit'] == '1') {
                    ...
                    if ($model->save() == true) {
										...
										$this->redirect(array('admintrouble/'.$page.''));
                ...
                if ($model->attachment1 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment1)) {
                    if ($model->attachment1 != Upload_file_common::getAttachmentById($model->id, 1,'trouble')) {
                ...
                if ($model->attachment2 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment2)) {
                    if ($model->attachment2 != Upload_file_common::getAttachmentById($model->id, 2,'trouble')) {
                ...
                if ($model->attachment3 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment3)) {
                    if ($model->attachment3 != Upload_file_common::getAttachmentById($model->id, 3,'trouble')) {
                ...
                $this->redirect(array('admintrouble/edit/?id=' . $model->id));
            }
        }
        else {
            $this->redirect(array('admintrouble/index'));
        }
        $this->render('/admin/trouble/editconfirm', array('model' => $model));
    }
    ...
    public function actionDetail() {
        ...
        $this->render('/admin/trouble/detail', array(
            'model' => $model,
                )
        );
    }
    ...
    public function loadModel() {
        if ($this->_trouble === null) {
            ...
            } else if (isset($_POST['Trouble'])) {
                $data = $_POST['Trouble'];
                $id = $data['id'];
                $this->_trouble = Trouble::model()->findbyPk(intval($id));
            } else {
     ...
     * Author:
     * User change:
     * Description: delete trouble object
     * */
    ...
    public function actionDelete() {
        ...
        $affected_trouble = $model->deleteByPk($id);
        ...
        if ($affected_trouble != $affected_update_information) {
            $transaction->rollback();
        } else {
            $transaction->commit();
        }
        if ($affected_trouble == 1) {
            if ($attachment1 != "" && file_exists(Yii::getPathOfAlias('webroot') . $attachment1)) {
                unlink(Yii::getPathOfAlias('webroot') . $attachment1);
